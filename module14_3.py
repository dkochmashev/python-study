...
async def show_info(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f'Версия {html.bold('0.14.3alpha')}'
    )
...
async def get_buying_list(message: Message, state: FSMContext):
    ...
    product = [
        {'label': 'Улыбнись!',
         ...
         'url': 'https://img5tv.cdnvideo.ru/webp/shared/files/202112/1_1449241.jpg'
         }, {
            'label': 'Развеселись!',
            'description': 'Для смелых поступков',
            'url': 'https://posamogonu.ru/wp-content/uploads/2019/05/foto-1.png'
        }, {
            'label': 'Погуляй!',
    ...
    for number in range(1, 5):
        await message.answer(
            f'Название: {product[number - 1]["label"]} | '
            f'Описание: {product[number - 1]["description"]} | '
            f'Цена: {number * 100}'
        )
        await message.answer_photo(photo=product[number - 1]["url"])
        product_buttons.append(InlineKeyboardButton(text=f'{product[number - 1]["label"]}',
                                                    callback_data=f'product_buying:{product[number - 1]["label"]}'))
